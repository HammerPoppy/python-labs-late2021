...
def string_search(P, T) -> List[int]:
    ...
    D = bad_character_table(P)
    L = good_suffix_table(P)
    F = full_shift_table(P)

    print('Допоміжний масив d:')
    print(D)
    print('\n')
    ...
    while k < len(T):
        ...
        if i == -1 or h == previous_k:
            ...
            char_shift = i - D[alphabet_index(T[h])][i]
            if i + 1 == len(P):
                suffix_shift = 1
            elif L[i + 1] == -1:
...
if __name__ == '__main__':
    print('Шукаємо rtyurtyuhjrtyu в ghjklujmghjklasdfghtgbyhnrtyurtyuhjrtyuyuiohjklrtyurtyuhjrtyuhjkl\n')
    matches = string_search('rtyurtyuhjrtyu', 'ghjklujmghjklasdfghtgbyhnrtyurtyuhjrtyuyuiohjklrtyurtyuhjrtyuhjkl')

    print('Знайдені входження:')
    print(matches)
